...
public class ISO8601Utils {
     ...
     */
    public static Date parse(@NonNull String date, ParsePosition pos) throws ParseException {
        ...
        try {
            ...
            int milliseconds = 0; // always use 0 otherwise returned date will include millis of
            // current time
    ...
    private static void checkOffset(String value, int offset, char expected) throws ParseException {
        ...
        if (found != expected) {
            throw new ParseException("Expected '" + expected + "' character but found '" + found
                    + "'", offset);
        }
    }
